...
<script>
    ...
    $(document).ready(function() {
        ...
        function showBannerDeletionModal(details) {
            var filesList = '';
            if (details.files_to_delete.length > 0) {
                filesList = '<div class="alert alert-info"><h6 class="alert-heading"><i class="fe fe-file mr-1"></i>Files to be Deleted</h6><ul class="mb-0">';
                details.files_to_delete.forEach(function(file) {
                    filesList += '<li><strong>' + file.type + ':</strong> ' + file.path + '</li>';
                });
                filesList += '</ul></div>';
            }
